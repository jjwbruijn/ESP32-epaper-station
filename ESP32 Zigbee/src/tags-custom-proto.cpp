...
{
    sprintf(buffer, "/state_%02X%02X%02X%02X%02X%02X%02X%02X.json", mac[7], mac[6], mac[5], mac[4], mac[3], mac[2],
        mac[1], mac[0]);
}
...
void processCheckin(const uint8_t* src, const struct CheckinInfo* ci)
{
    ...
    sprintf(sbuffer, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", src[7], src[6], src[5], src[4], src[3], src[2], src[1],
        src[0]);
#ifdef NETWORK_MODE
...
{
    Serial.printf("Tag %dx%d (%dx%dmm)\n", taginfo->screenPixWidth, taginfo->screenPixHeight, taginfo->screenMmWidth,
...
void parsePacket(const uint8_t* src, const uint8_t* data, const uint8_t len)
{
    ...
    else if (data[0] == PKT_ASSOC_RESP)
    { // not relevant for a base
