...
// look for the latest version of the firmware file... It's supposed to be
// something like zigbeebase0003.bin
...
// extract original mac from firmware and make it 2 bytes longer based on info
// in settings.h
...
void performDeviceFlash()
{
    ...
    if (!interfaceWorking)
    {
        Serial.print("I wasn't able to connect to a ZBS tag, please check wiring "
                     "and definitions in the settings.h file.\n");
        return;
    }
    ...
    if (*((uint64_t*)(mac)) == 0xFFFFFFFFFFFFFFFF)
    {
        ...
        *((uint64_t*)(mac)) = getOriginalTagMac();
        ...
        for (uint8_t c = 0; c < 8; c++)
        {
            ...
            // write mac directly to infoblock without erasing; the bytes should all
            // be 0xFF anyway
