...
struct AssocInfo
{
    ...
    uint32_t retryDelay; // if download fails mid-way wait thi smany msec to retry
...
struct CheckinInfo
{
    ...
    uint8_t temperature; // zero if not reported/not supported to be reported. else,
                         // this minus CHECKIN_TEMP_OFFSET is temp in degrees C
...
struct ChunkInfo
{
    ...
    uint8_t data[]; // no data means request is out of bounds of this version no
                    // longer exists
} __packed;
